...
define('J_SHORTCODES_VERSION',  '1.402');
...
[jgallery
   ...
   newpage="*0"                                          // "1" => open new browser windows when clicking on links in rich HTML panel
    ...
    [jgal/]
    /public_html/wp/_FILES/demo*.jpg         // Must start with '/' and contain at least one '*' in it. Anything within {XXX} will be interpreted as regex pattern for '|XXX|' filespec match.
    [jgal/]
...
//    'overlay_position'      =>    '"bottom"',             // STRING - position of panel overlay (bottom, top, left, right)
   ...
   // Examples: "800x600|80x60"  or "800x600:fill=0"  or "|80x60:pos=top,fill=1,all=1"
   $allp = explode ('|', $wh);
   ...
   if (@$icon_params_arr['fill'])
      $js_default_params['frame_scale'] = '"crop"';
   else
      $js_default_params['frame_scale'] = '"nocrop"';
   //---------------------------------------
   ...
   foreach ($content_arr as $idx=>$content_el)
      ...
      $image_url        =  trim (strip_tags($img_data[0]));
      ...
      if (strpos($image_url, "http")===0)
         ...
         $image_extra_content    =  trim (strip_tags(@$img_data[1]));
         $image_title_tag        =  trim (strip_tags(@$img_data[2]));

         if ($image_title_tag)
            {
            $image_title_tag = "title=\"{$image_title_tag}\"";
         ...
         if ($extra_content_type == "content")
            {
            // If images URL were specified, but html="1" (use rich HTML content flag set) then create extra piece for image
            $image_height_tag  = 'height="98%"';

            $image_extra_content = "<img src=\"{$image_url}\" {$image_height_tag} {$image_alt_tag} {$image_title_tag} />";
            }

         if ($image_extra_content)
            {
            $li_element =<<<TTT
               <li>
...
TTT;
      ...
      // Gallery element: blog posts
         ...
         $wp_query_params = trim (@$img_data[1]);
         if (!$wp_query_params)
...
TTT;
      ...
      // Gallery element - list of image files at physical location
         ...
         {
         // Create params for: limit, order by (name, date, ASC|DESC, random)
         // directory with images specified. Loop through them.
         $extra_content_type = "overlay"; // Extra stuff after image URL is dark overlay text on top of picture
...
TTT;
   ...
   $j_gallery_html =<<<TTT
      <div class="jgallery" style="visibility:hidden;">
         {$js_template}
